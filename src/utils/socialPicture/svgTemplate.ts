...
const backgroundImageWithNoOverlay = `
  <rect width="512" height="512" fill="url(#backImg)" filter="url(#shadowy)"/>
`;

const backgroundImageWithShadowFilter = `
  <rect width="512" height="512" fill="url(#backImg)" filter="url(#shadowy)"/>
...
export const determineShortLabel = (domainLabel: string): string => {
  let shortLabel = '';
  if (domainLabel.length > 40) {
    shortLabel = domainLabel.substring(0, 40 - 3) + '...';
  }

  return shortLabel;
};

export const createOverlayText = (
  fontSize: number,
  label: string,
  extension: string,
) => {
...
export const simpleSVGTemplate = (href: string) => `
  <svg
    width="512"
    height="512"
    viewBox="0 0 512 512"
  ...
  >
    <image href="${href}" width="512" height="512" preserveAspectRatio="none"/>
  </svg>
`;
...
export const offChainSVGTemplate = (
  ...
  const shortLabel = determineShortLabel(domain.label);
  return `
    <svg width="512" height="512" viewBox="0 0 512 512" fill="none" xmlns="http://www.w3.org/2000/svg">
      ...
      ${
        withOverlay
          ...
          : backgroundImageWithNoOverlay
      }
      ${
        withOverlay
          ? createOverlayText(
              fontSize,
              shortLabel || domain.label,
              domain.extension,
            )
          : null
      }
